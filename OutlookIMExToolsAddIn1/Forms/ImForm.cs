...
namespace OutlookIMExToolsAddIn1.Forms
{
    public partial class ImForm : Form
    {
        ...
        )
        {
            ...
            var converterSession = _thunderbirdHelperUsecase.CreateConverterSession();
            try
            {
                var emptyMail = _thunderbirdHelperUsecase.LoadPseudoReceived();
                try
                {
                    ...
                    )
                    {
                        foreach (var tbNode in tbNodes2)
                        {
                            ...
                            var subFolder = _thunderbirdHelperUsecase.AddOrGetFolder(folder2.Folders, tbNode.DisplayName);
